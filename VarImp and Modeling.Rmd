...
varlist2=  list(c(4:6, 8:40,63:71),  c(4,6, 8:40,63:71,74), c(4:5, 8:40,63:71,74), c(4:6, 8:40,63:67, 69:71,74), c(4:6, 8:40,64:71,74), c(4:6, 8:40,63:70,74), c(4:6, 8:40,63:65, 67:71,74))
...
for (j in 1) {
  ...
  for (i in 1:2){
    ...
    train_x = model.matrix(kickReturnYardage ~ ., data = train.h[,c(4:40,63:68,70,71 )])[,-1]
...
#Average MAE of 4.519 - c(4:6, 8:40,63:71 )
#Average MAE of 4.518 - c(4:40,63:68,70,71 )
...
for (i in 1:2){
  ...
  val_x = model.matrix(kickReturnYardage ~ ., data = val.h[,c(4:40,63:68,70,71 )])[,-1]
...
#Based on Areas
```{r}
full.a1 = full_set %>% filter(return_zone %in% c(1,2,3))
full.a2 = full_set %>% filter(return_zone %in% c(4,5,6))
full.a3 = full_set %>% filter(return_zone %in% c(7,8,9))
...
for (i in 1:3){
  zones = ifelse(i == 1, list(1:3), ifelse(i == 2, list(4:6), list(7:9)))
  full.a = full_set %>% filter(return_zone %in% zones[[1]])
  
  full_x = model.matrix(kickReturnYardage ~ ., data = full.a[,c(4:6, 8:40,63:68, 70, 71 )])[,-1]
  full_y = full.a$kickReturnYardage
  
  full.a$xgb_area_pred = predict(area_xgbs[i], type = "response", full_x)[[1]]
  full_with_preds = rbind(full.a, full_with_preds)
...
```{r}
full_with_preds$yard_dif = full_with_preds$kickReturnYardage - full_with_preds$xgb_area_pred
...
#Based on Zones
```{r}
full.z1 = full_set %>% filter(return_zone == 1)
full.z2 = full_set %>% filter(return_zone == 2)
full.z3 = full_set %>% filter(return_zone == 3)
full.z4 = full_set %>% filter(return_zone == 4)
full.z5 = full_set %>% filter(return_zone == 5)
full.z6 = full_set %>% filter(return_zone == 6)
full.z7 = full_set %>% filter(return_zone == 7)
full.z8 = full_set %>% filter(return_zone == 8)
full.z9 = full_set %>% filter(return_zone == 9)
...
for (i in 1:9){
  ...
  full.z = full_set %>% filter(return_zone == i)
  
  full_x = model.matrix(kickReturnYardage ~ ., data = full.z[,c(4:6, 8:40,52:64,68:71 )])[,-1]
  full_y = full.z$kickReturnYardage
  
  full.z$xgb_zone_pred = predict(zone_xgbs[i], type = "response", full_x)[[1]]
  full_with_preds = rbind(full.z, full_with_preds)
...
```{r}
full_with_preds$yard_dif = full_with_preds$kickReturnYardage - full_with_preds$xgb_zone_pred
...
#Use area zones and still cut down on amount of variables used
#Ideally you want at least 10 observations per feature
