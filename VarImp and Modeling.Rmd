...
```{r}
train_x = model.matrix(kickReturnYardage ~ ., data = train.z2[,c(4:51, 55:60)])[,-1]
train_y = train.z2$kickReturnYardage
...
for (i in 1:9){
  ...
  set.seed(123)
  xgb.punt.final = xgboost(data = train_x, label = train_y, 
                           subsample = xgb.punt.caret$bestTune$subsample, nrounds = 4, 
                           eta = xgb.punt.caret$bestTune$eta,
                           max_depth = xgb.punt.caret$bestTune$max_depth, prediction = T)


  #x = xgb.importance(feature_names = colnames(train_x), model = xgb.punt.final)
  #print(x)
  g = xgb.ggplot.importance(xgb.importance(feature_names = colnames(train_x), model = xgb.punt.final)) + ggtitle(paste("Feature Importance Zone", i))
  print(g)
}
