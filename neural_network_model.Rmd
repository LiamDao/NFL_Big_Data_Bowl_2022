...
## All Variables and Outlier Flag
```{r}
################# Now try with using all variabels (not the var.sel) ########################
outliersall <- boxplot(train$kickReturnYardage, plot=FALSE)$out
train['outlier_flag'] <- ifelse(train$kickReturnYardage %in% outliersall, "1", "0")

# Model Tuning
tune_grid <- expand.grid(
  .size = c(1, 2, 3, 4, 5),
  .decay = seq(.65, .75, by = .01)
)
...
## Use Models on Validation
```{r validation}
# Model with subsetted variables and outlier flag
set.seed(123)
final.subset.model <- nnet(kickReturnYardage ~ . -gameId -playId, data = train6, size = 6, decay = .945, linout = TRUE )

final.subset.predict <- predict(final.subset.model, newdata = val2)

# Calculate RMSE and MAE
final.subset.rmse <- sqrt(mean((final.subset.predict - val2$kickReturnYardage)^2)) # RMSE
final.subset.mae <- mae(val2$kickReturnYardage, final.subset.predict) # MAE
```
> Performance Metrics:
RMSE: 7.93
MAE: 5.93
